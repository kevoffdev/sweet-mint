...
export const CartModal = ({isOpen, onClose}: CartModalProps) => {
  const {productsCart, removeProduct, addProduct, resetCart} = useCart();
  ...
  const handleClickSum = (id: IProduct["id"]) => {
  ...
  const handleClickMinus = (id: IProduct["id"]) => {
    if (productsCart[id].quantity - 1 < 1) return;
    addProduct({id, quantity: -1});
  };

  const handleBuyProducts = async () => {
    const productCartKeys = Object.keys(productsCart);

    const {productsOrder, totalAmount} = products.reduce(
      (
        acc: {
          productsOrder: {productId: string; quantity: number; price: number}[];
          totalAmount: number;
        },
        product,
      ) => {
        if (productCartKeys.includes(product.id)) {
          const quantity = productsCart[product.id].quantity;
          const productTotal = product.price * quantity;

          // Agrega el producto filtrado al array
          acc.productsOrder.push({
            productId: product.id,
            quantity,
            price: Number(product.price),
          });

          // Acumula el totalAmount
          acc.totalAmount += productTotal;
        }

        return acc;
      },
      {productsOrder: [], totalAmount: 0}, // Objeto inicial con array y totalAmount
    );
        ...
        <div ref={modalRef} className="relative w-[480px] overflow-auto bg-white p-6 shadow-lg">
          ...
          {productsCartLength > 0 ? (
            <section className="flex flex-col gap-4">
              ...
              <button
                className="mt-5 rounded-full bg-black p-4 text-white"
